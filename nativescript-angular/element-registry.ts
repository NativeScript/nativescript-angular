...
const elementMap = new Map<
  string,
  { resolver: ViewResolver; meta?: ViewClassMeta }
>();
...
export function getViewMeta(nodeName: string): ViewClassMeta {
  const entry =
    elementMap.get(nodeName) || elementMap.get(nodeName.toLowerCase());
...
export function getSingleViewRecursive(
  nodes: Array<any>,
  nestLevel: number
): View {
  ...
  if (actualNodes.length === 0) {
    throw new Error(
      `No suitable views found in list template! ` +
        `Nesting level: ${nestLevel}`
    );
  } else if (actualNodes.length > 1) {
    throw new Error(
      `More than one view found in list template!` +
        `Nesting level: ${nestLevel}`
    );
  }
